		...
		observe	: function(subject, topic, data) {
			if (topic == "xpcom-shutdown") {
				if (base.config["removeBrowserProfile"]) {
					...
					let profiles = tps.profiles;
					//sl.debug(tps.selectedProfile.rootDir.path);
					while(profiles.hasMoreElements()) {
						let profile = profiles.getNext();
						profile.QueryInterface(Ci.nsIToolkitProfile);
						try {
							sl.debug("remove: " + profile.rootDir.path);
							profile.lock({unlocker});
							sl.debug(unlocker);
							profile.remove(true);
							//entry.remove(true);
						}
